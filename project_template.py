...
# Exercise-2 TODOs:
    ...
    ##filename = 'pr2_view.pcd'
    ##pcl.save(pr2_view, filename)
    ...
    #filename = 'RANSAC_plane_fitting.pcd'
...
def pr2_mover(objects_list):
	...
	#drop_x_coord.append(drop_pos[0])
    	#drop_y_coord.append(drop_pos[1])
    	#drop_z_coord.append(drop_pos[2])
	#if object_group == 'green'
	 #   which_arm = 'right'
	#else
	 #   which_arm = 'left'
	...
	for object in objects_list:
	    ...
	    centroids.append(np.mean(points_arr, axis=0, dtype=np.float32)[:3])
	    #centroids = [type(np.asscalar(i)) for i in centroids]
	    #centroids = type(np.asscalar(centroids))
	    #p = np.array([centroids[1]])
	    #print(p)
	    #c_n = np.asscalar(p)
	    #print(c_n)
            #center_y = type(np.asscalar(centroids([1])))
            #center_z = type(np.asscalar(centroids([2])))
	    #centroids[1] = c_n
            #centroids[1] = center_y
            #centroids[2] = center_z
